...
TEST(MaterialTest, TestMaterialAccess) {
  ...
  draco_illixr::Material material;

  material.SetName("Superalloy");
  ASSERT_EQ(material.GetName(), "Superalloy");
  material.SetColorFactor(draco_illixr::Vector4f(1.f, 0.2f, 0.1f, 0.9f));
  ASSERT_EQ(material.GetColorFactor(), draco_illixr::Vector4f(1.f, 0.2f, 0.1f, 0.9f));
  ...
  material.SetEmissiveFactor(draco_illixr::Vector3f(0.2f, 0.f, 0.1f));
  ASSERT_EQ(material.GetEmissiveFactor(), draco_illixr::Vector3f(0.2f, 0.f, 0.1f));
  ...
  material.SetSheenColorFactor(draco_illixr::Vector3f(0.4f, 0.2f, 0.8f));
  ASSERT_EQ(material.GetSheenColorFactor(), draco_illixr::Vector3f(0.4f, 0.2f, 0.8f));
  ...
  material.SetAttenuationColor(draco_illixr::Vector3f(0.2f, 0.5f, 0.8f));
  ASSERT_EQ(material.GetAttenuationColor(), draco_illixr::Vector3f(0.2f, 0.5f, 0.8f));
  ...
  material.SetSpecularColorFactor(draco_illixr::Vector3f(0.4f, 1.f, 1.f));
  ASSERT_EQ(material.GetSpecularColorFactor(), draco_illixr::Vector3f(0.4f, 1.f, 1.f));

  ASSERT_EQ(material.GetTextureMapByType(draco_illixr::TextureMap::COLOR), nullptr);
  ...
  material.SetTextureMap(std::move(texture), draco_illixr::TextureMap::COLOR, 0);

  ASSERT_NE(material.GetTextureMapByType(draco_illixr::TextureMap::COLOR), nullptr);
  ASSERT_EQ(material.NumTextureMaps(), 1);
  ASSERT_EQ(material.GetTextureMapByIndex(0),
            material.GetTextureMapByType(draco_illixr::TextureMap::COLOR));

  std::unique_ptr<draco_illixr::Texture> texture2 =
      draco_illixr::ReadTextureFromFile(draco_illixr::GetTestFileFullPath("test.png"))
          .value();
  ASSERT_NE(texture2, nullptr);
  material.SetTextureMap(std::move(texture2), draco_illixr::TextureMap::EMISSIVE, 1);

  ASSERT_NE(material.GetTextureMapByType(draco_illixr::TextureMap::EMISSIVE), nullptr);
  ASSERT_EQ(material.GetTextureMapByType(draco_illixr::TextureMap::EMISSIVE)
                ->tex_coord_index(),
            1);
  ...
  std::unique_ptr<draco_illixr::Texture> texture3 =
      draco_illixr::ReadTextureFromFile(draco_illixr::GetTestFileFullPath("test.png"))
          .value();
  ASSERT_NE(texture3, nullptr);
  material.SetTextureMap(std::move(texture2), draco_illixr::TextureMap::EMISSIVE, 2);
  ASSERT_EQ(material.NumTextureMaps(), 2);
  ASSERT_EQ(material.GetTextureMapByType(draco_illixr::TextureMap::EMISSIVE)
                ->tex_coord_index(),
            2);

  std::unique_ptr<draco_illixr::TextureMap> texture_map4(new draco_illixr::TextureMap());
  std::unique_ptr<draco_illixr::Texture> texture4 =
      draco_illixr::ReadTextureFromFile(draco_illixr::GetTestFileFullPath("test.png"))
          .value();
  material.SetTextureMap(std::move(texture4), draco_illixr::TextureMap::ROUGHNESS, 0);
  ASSERT_EQ(material.NumTextureMaps(), 3);
  ASSERT_NE(material.GetTextureMapByType(draco_illixr::TextureMap::ROUGHNESS),
            nullptr);

  material.SetTransparencyMode(draco_illixr::Material::TRANSPARENCY_BLEND);
  ASSERT_EQ(material.GetTransparencyMode(),
            draco_illixr::Material::TRANSPARENCY_BLEND);
...
TEST(MaterialTest, TestMaterialCopy) {
  ...
  material.SetColorFactor(draco_illixr::Vector4f(0.3f, 0.2f, 0.4f, 0.9f));
  material.SetMetallicFactor(0.2f);
  material.SetRoughnessFactor(0.4f);
  material.SetEmissiveFactor(draco_illixr::Vector3f(0.3f, 0.1f, 0.2f));
  material.SetTransparencyMode(draco_illixr::Material::TRANSPARENCY_MASK);
  ...
  std::unique_ptr<draco_illixr::Texture> texture =
      draco_illixr::ReadTextureFromFile(draco_illixr::GetTestFileFullPath("test.png"))
          .value();
  ASSERT_NE(texture, nullptr);
  material.SetTextureMap(std::move(texture), draco_illixr::TextureMap::EMISSIVE, 2);

  draco_illixr::Material new_material;
  ...
  for (int i = 0; i < draco_illixr::TextureMap::TEXTURE_TYPES_COUNT; ++i) {
    const draco_illixr::TextureMap::Type texture_map_type =
        static_cast<draco_illixr::TextureMap::Type>(i);
  ...
  for (int i = 0; i < material.NumTextureMaps(); ++i) {
    const draco_illixr::TextureMap *const tm0 = material.GetTextureMapByIndex(i);
    const draco_illixr::TextureMap *const tm1 = new_material.GetTextureMapByIndex(i);
...
TEST(MaterialTest, RemoveTextureMap) {
  ...
  std::unique_ptr<draco_illixr::Texture> texture_2 =
      draco_illixr::ReadTextureFromFile(draco_illixr::GetTestFileFullPath("test.png"))
          .value();

  material.SetTextureMap(std::move(texture), draco_illixr::TextureMap::EMISSIVE, 0);

  ASSERT_EQ(material.NumTextureMaps(), 2);

  // Try to delete the color texture.
  std::unique_ptr<draco_illixr::TextureMap> removed_texture =
      material.RemoveTextureMapByType(draco_illixr::TextureMap::COLOR);
  ASSERT_NE(removed_texture, nullptr);
  ASSERT_EQ(removed_texture->type(), draco_illixr::TextureMap::COLOR);
  ASSERT_EQ(material.NumTextureMaps(), 1);
  ASSERT_NE(material.GetTextureMapByType(draco_illixr::TextureMap::EMISSIVE), nullptr);
  ASSERT_EQ(material.GetTextureMapByIndex(0)->type(),
            draco_illixr::TextureMap::EMISSIVE);
  ASSERT_EQ(material.GetTextureMapByType(draco_illixr::TextureMap::COLOR), nullptr);

  removed_texture = material.RemoveTextureMapByIndex(0);
  ASSERT_NE(removed_texture, nullptr);
  ASSERT_EQ(removed_texture->type(), draco_illixr::TextureMap::EMISSIVE);
  ASSERT_EQ(material.NumTextureMaps(), 0);
  ASSERT_EQ(material.GetTextureMapByType(draco_illixr::TextureMap::EMISSIVE), nullptr);
}
...
TEST(MaterialTest, SharedTexture) {
  ...
  std::unique_ptr<draco_illixr::Texture> texture =
      draco_illixr::ReadTextureFromFile(draco_illixr::GetTestFileFullPath("test.png"))
          .value();
  ASSERT_NE(texture, nullptr);
  draco_illixr::Texture *texture_raw = texture.get();
  material.SetTextureMap(std::move(texture), draco_illixr::TextureMap::COLOR, 0);

  DRACO_ASSERT_OK(
      material.SetTextureMap(texture_raw, draco_illixr::TextureMap::EMISSIVE, 0));

  ASSERT_EQ(material.NumTextureMaps(), 2);

  // Read a new texture.
  texture = draco_illixr::ReadTextureFromFile(draco_illixr::GetTestFileFullPath("test.png"))
                .value();
  // Texture is not owned by the material so we expect a failure.
  ASSERT_FALSE(
      material
          .SetTextureMap(texture.get(), draco_illixr::TextureMap::AMBIENT_OCCLUSION, 0)
          .ok());
}
