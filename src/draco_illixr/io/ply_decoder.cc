...
void PlyDecoder::DecodeMerge(Mesh &new_mesh){
	//Code picked from DecodeFaceData
	//Update face count
	unsigned old_num_faces = out_mesh_->num_faces();
	unsigned new_faces = new_mesh.num_faces();
	printf("existing mesh has %u faces, new mesh has %u faces\n", old_num_faces, new_faces);
	out_mesh_->SetNumFaces(old_num_faces + new_faces);
	...
	//for(FaceIndex face_index(out_num_faces); face_index < out_mesh->num_faces(); ++face_index){
        //        float dracoVertex_v1[3];
        ...
        //        uint8_t dracoColors_v1[3];
        //        uint8_t dracoColors_v2[3];
        //        uint8_t dracoColors_v3[3];
...
Status PlyDecoder::DecodeExternal(const PlyReader &ply_reader, bool skip_deduplication) {
  ...
  const PlyProperty *voxelblock_z = face_element->GetPropertyByName("vb_z");
  if (voxelblock_x == nullptr || voxelblock_y == nullptr || voxelblock_z == nullptr) {
	  printf("voxel block info not found\n");
  }else{
...
Status PlyDecoder::DecodeInternal() {
  ...
  printf("ply_reader elements_size(): %lu\b",ply_reader.elements_.size());
  return OkStatus();
}
...
//#pragma omp parallel for
  for (int i = 0; i < num_polygons; ++i) {
    ...
    for (int64_t ti = 0; ti < num_triangles; ++ti) {
      ...
      //printf("face[0]: %d, face[1]: %d, face[2]: %d\n",face[0].value(), face[1].value(), face[2].value());
...
Status PlyDecoder::DecodeVertexData(const PlyElement *vertex_element) {
  ...
  // Decode vertex positions.
  {
    ...
    if (dt == DT_FLOAT32) {
      ...
      //printf("int32 detected\n");
      ReadPropertiesToAttribute<int32_t>(
          properties, out_point_cloud_->attribute(att_id), num_vertices);
    }
    //pyh: add support for 64bit float
    else if (dt == DT_FLOAT64) {
      printf("float64 detected SHOULD NOT BE TRIGGERED\n");
