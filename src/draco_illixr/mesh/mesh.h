...
class Mesh : public PointCloud {
 public:
  ...
  //pyh
  void AddVB(const VoxelBlock &VB) { voxelblocks_.push_back(VB); }
  ...
  const VoxelBlock &voxelblock(FaceIndex face_id) const {
    DRACO_DCHECK_LE(0, face_id.value());
    DRACO_DCHECK_LT(face_id.value(), static_cast<int>(voxelblocks_.size()));
    return voxelblocks_[face_id];
  }
