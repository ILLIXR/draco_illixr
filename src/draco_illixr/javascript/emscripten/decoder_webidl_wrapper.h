...
typedef draco_illixr::DataType draco_DataType;
...
class MetadataQuerier {
 public:
  ...
  bool HasEntry(const draco_illixr::Metadata &metadata, const char *entry_name) const;

  // This function does not guarantee that entry's type is long.
  long GetIntEntry(const draco_illixr::Metadata &metadata,
                   const char *entry_name) const;

  // This function does not guarantee that entry types are long.
  void GetIntEntryArray(const draco_illixr::Metadata &metadata, const char *entry_name,
                        DracoInt32Array *out_values) const;

  // This function does not guarantee that entry's type is double.
  double GetDoubleEntry(const draco_illixr::Metadata &metadata,
                        const char *entry_name) const;

  // This function does not guarantee that entry's type is char*.
  const char *GetStringEntry(const draco_illixr::Metadata &metadata,
                             const char *entry_name);

  long NumEntries(const draco_illixr::Metadata &metadata) const;
  const char *GetEntryName(const draco_illixr::Metadata &metadata, int entry_id);

 private:
  // Cached values for metadata entries.
  std::vector<std::string> entry_names_;
  const draco_illixr::Metadata *entry_names_metadata_;
...
class Decoder {
 public:
  ...
  const draco_illixr::Status *DecodeBufferToPointCloud(
      draco_illixr::DecoderBuffer *in_buffer, draco_illixr::PointCloud *out_point_cloud);

  // Decodes a point cloud from the provided array.
  const draco_illixr::Status *DecodeArrayToPointCloud(
      const char *data, size_t data_size, draco_illixr::PointCloud *out_point_cloud);

  // Decodes a triangular mesh from the provided buffer.
  // Deprecated: Use DecodeArrayToMesh.
  const draco_illixr::Status *DecodeBufferToMesh(draco_illixr::DecoderBuffer *in_buffer,
                                          draco_illixr::Mesh *out_mesh);

  // Decodes a mesh from the provided array.
  const draco_illixr::Status *DecodeArrayToMesh(const char *data, size_t data_size,
                                         draco_illixr::Mesh *out_mesh);

  // Returns an attribute id for the first attribute of a given type.
  long GetAttributeId(const draco_illixr::PointCloud &pc,
  ...
  static long GetAttributeIdByName(const draco_illixr::PointCloud &pc,
                                   const char *attribute_name);

  // Returns an attribute id of an attribute with a specified metadata pair
  // <|metadata_name|, |metadata_value|>.
  static long GetAttributeIdByMetadataEntry(const draco_illixr::PointCloud &pc,
                                            const char *metadata_name,
                                            const char *metadata_value);

  // Returns an attribute id of an attribute that has the unique id.
  static const draco_illixr::PointAttribute *GetAttributeByUniqueId(
      const draco_illixr::PointCloud &pc, long unique_id);

  // Returns a PointAttribute pointer from |att_id| index.
  static const draco_illixr::PointAttribute *GetAttribute(const draco_illixr::PointCloud &pc,
                                                   long att_id);

  // Returns Mesh::Face values in |out_values| from |face_id| index.
  static bool GetFaceFromMesh(const draco_illixr::Mesh &m,
  ...
  static long GetTriangleStripsFromMesh(const draco_illixr::Mesh &m,
  ...
  static bool GetTrianglesUInt16Array(const draco_illixr::Mesh &m, int out_size,
                                      void *out_values);
  static bool GetTrianglesUInt32Array(const draco_illixr::Mesh &m, int out_size,
                                      void *out_values);
  ...
  static bool GetAttributeFloat(
      ...
      draco_illixr::AttributeValueIndex::ValueType entry_index,
  ...
  static bool GetAttributeFloatForAllPoints(const draco_illixr::PointCloud &pc,
                                            const draco_illixr::PointAttribute &pa,
                                            DracoFloat32Array *out_values);
  ...
  // |out_size| is the size in bytes of |out_values|. |out_values| must be
  // allocated before calling this function.
  static bool GetAttributeDataArrayForAllPoints(const draco_illixr::PointCloud &pc,
                                                const draco_illixr::PointAttribute &pa,
  ...
  const draco_illixr::Metadata *GetMetadata(const draco_illixr::PointCloud &pc) const;
  const draco_illixr::Metadata *GetAttributeMetadata(const draco_illixr::PointCloud &pc,
                                              long att_id) const;

 private:
  template <class DracoArrayT, class ValueTypeT>
  static bool GetAttributeDataForAllPoints(const draco_illixr::PointCloud &pc,
                                           const draco_illixr::PointAttribute &pa,
                                           draco_illixr::DataType draco_signed_type,
                                           draco_illixr::DataType draco_unsigned_type,
    ...
    if ((pa.data_type() == draco_signed_type ||
      ...
      const ValueTypeT *ptr = reinterpret_cast<const ValueTypeT *>(
          pa.GetAddress(draco_illixr::AttributeValueIndex(0)));
  ...
  static bool GetAttributeDataArrayForAllPoints(const draco_illixr::PointCloud &pc,
                                                const draco_illixr::PointAttribute &pa,
                                                const draco_illixr::DataType type,
                                                int out_size,
                                                void *out_values) {
    const int components = pa.num_components();
    const int num_points = pc.num_points();
    ...
    if (requested_type_matches && pa.is_mapping_identity()) {
      // Copy values directly to the output vector.
      const auto ptr = pa.GetAddress(draco_illixr::AttributeValueIndex(0));
