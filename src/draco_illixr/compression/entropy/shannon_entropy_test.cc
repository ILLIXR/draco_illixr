...
TEST(ShannonEntropyTest, TestBinaryEntropy) {
  ...
  ASSERT_EQ(draco_illixr::ComputeBinaryShannonEntropy(0, 0), 0);
  ASSERT_EQ(draco_illixr::ComputeBinaryShannonEntropy(10, 0), 0);
  ASSERT_EQ(draco_illixr::ComputeBinaryShannonEntropy(10, 10), 0);
  ASSERT_NEAR(draco_illixr::ComputeBinaryShannonEntropy(10, 5), 1.0, 1e-4);
}
...
TEST(ShannonEntropyTest, TestStreamEntropy) {
  ...
  draco_illixr::ShannonEntropyTracker entropy_tracker;
  ...
  for (int i = 0; i < symbols.size(); ++i) {
    ...
    ASSERT_EQ(draco_illixr::ShannonEntropyTracker::GetNumberOfDataBits(entropy_data),
              stream_entropy_bits);

    // Make sure the entropy is approximately the same as the one we compute
    // directly from all symbols.
    const int64_t expected_entropy_bits = draco_illixr::ComputeShannonEntropy(
  ...
  draco_illixr::ShannonEntropyTracker entropy_tracker_2;
