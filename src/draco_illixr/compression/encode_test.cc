...
class EncodeTest : public ::testing::Test {
 protected:
  EncodeTest() {}
  std::unique_ptr<draco_illixr::Mesh> CreateTestMesh() const {
    draco_illixr::TriangleSoupMeshBuilder mesh_builder;
    ...
    const int32_t pos_att_id = mesh_builder.AddAttribute(
        draco_illixr::GeometryAttribute::POSITION, 3, draco_illixr::DT_FLOAT32);
    const int32_t tex_att_id_0 = mesh_builder.AddAttribute(
        draco_illixr::GeometryAttribute::TEX_COORD, 2, draco_illixr::DT_FLOAT32);
    ...
    mesh_builder.SetAttributeValuesForFace(
        pos_att_id, draco_illixr::FaceIndex(0), draco_illixr::Vector3f(0.f, 0.f, 0.f).data(),
        draco_illixr::Vector3f(1.f, 0.f, 0.f).data(),
        draco_illixr::Vector3f(1.f, 1.f, 0.f).data());
    mesh_builder.SetAttributeValuesForFace(
        tex_att_id_0, draco_illixr::FaceIndex(0), draco_illixr::Vector2f(0.f, 0.f).data(),
        draco_illixr::Vector2f(1.f, 0.f).data(), draco_illixr::Vector2f(1.f, 1.f).data());
    mesh_builder.SetAttributeValuesForFace(
        tex_att_id_1, draco_illixr::FaceIndex(0), draco_illixr::Vector2f(0.f, 0.f).data(),
        draco_illixr::Vector2f(1.f, 0.f).data(), draco_illixr::Vector2f(1.f, 1.f).data());

    return mesh_builder.Finalize();
  }

  std::unique_ptr<draco_illixr::PointCloud> CreateTestPointCloud() const {
    draco_illixr::PointCloudBuilder pc_builder;
    ...
    const int32_t gen_att_id_0 = pc_builder.AddAttribute(
        draco_illixr::GeometryAttribute::GENERIC, kNumGenAttCoords0, draco_illixr::DT_UINT32);
    const int32_t gen_att_id_1 = pc_builder.AddAttribute(
        draco_illixr::GeometryAttribute::GENERIC, kNumGenAttCoords1, draco_illixr::DT_UINT8);

    std::vector<uint32_t> gen_att_data_0(kNumGenAttCoords0);
    std::vector<uint32_t> gen_att_data_1(kNumGenAttCoords1);

    // Initialize the attribute values.
    for (draco_illixr::PointIndex i(0); i < kNumPoints; ++i) {
      const float pos_coord = static_cast<float>(i.value());
      pc_builder.SetAttributeValueForPoint(
          pos_att_id, i,
          draco_illixr::Vector3f(pos_coord, -pos_coord, pos_coord).data());
  ...
  std::unique_ptr<draco_illixr::PointCloud> CreateTestPointCloudPosNorm() const {
    draco_illixr::PointCloudBuilder pc_builder;
    ...
    const int32_t norm_att_id = pc_builder.AddAttribute(
        draco_illixr::GeometryAttribute::NORMAL, 3, draco_illixr::DT_FLOAT32);
    ...
    for (draco_illixr::PointIndex i(0); i < kNumPoints; ++i) {
      ...
      draco_illixr::Vector3f norm(pos_coord * 2.f, pos_coord - 2.f, pos_coord * 3.f);
      norm.Normalize();
  ...
  int GetQuantizationBitsFromAttribute(const draco_illixr::PointAttribute *att) const {
    if (att == nullptr) {
      return -1;
    }
    draco_illixr::AttributeQuantizationTransform transform;
  ...
  void VerifyNumQuantizationBits(const draco_illixr::EncoderBuffer &buffer,
    ...
    decoder.SetSkipAttributeTransform(draco_illixr::GeometryAttribute::TEX_COORD);

    draco_illixr::DecoderBuffer in_buffer;
  ...
  void TestNumberOfEncodedEntries(const std::string &file_name,
    ...
    std::unique_ptr<draco_illixr::PointCloud> geometry;
    draco_illixr::Mesh *mesh = nullptr;

    if (encoding_method == draco_illixr::MESH_EDGEBREAKER_ENCODING ||
        encoding_method == draco_illixr::MESH_SEQUENTIAL_ENCODING) {
      std::unique_ptr<draco_illixr::Mesh> mesh_tmp =
          draco_illixr::ReadMeshFromTestFile(file_name);
      mesh = mesh_tmp.get();
      ...
      geometry = draco_illixr::ReadPointCloudFromTestFile(file_name);
    }
    ASSERT_NE(mesh, nullptr);

    draco_illixr::Encoder encoder;
    encoder.SetAttributeQuantization(draco_illixr::GeometryAttribute::POSITION, 14);
    encoder.SetAttributeQuantization(draco_illixr::GeometryAttribute::TEX_COORD, 12);
    encoder.SetAttributeQuantization(draco_illixr::GeometryAttribute::NORMAL, 10);
...
TEST_F(EncodeTest, TestExpertEncoderQuantization) {
  ...
  draco_illixr::ExpertEncoder encoder(*mesh);
...
TEST_F(EncodeTest, TestEncoderQuantization) {
  ...
  draco_illixr::Encoder encoder;
  encoder.SetAttributeQuantization(draco_illixr::GeometryAttribute::POSITION, 16);
  encoder.SetAttributeQuantization(draco_illixr::GeometryAttribute::TEX_COORD, 15);
...
TEST_F(EncodeTest, TestLinesObj) {
  ...
  std::unique_ptr<draco_illixr::Mesh> mesh(
      draco_illixr::ReadMeshFromTestFile("test_lines.obj"));
  ASSERT_NE(mesh, nullptr);
  ASSERT_EQ(mesh->num_faces(), 0);
  std::unique_ptr<draco_illixr::PointCloud> pc(
      draco_illixr::ReadPointCloudFromTestFile("test_lines.obj"));
...
TEST_F(EncodeTest, TestQuantizedInfinity) {
  ...
  std::unique_ptr<draco_illixr::PointCloud> pc(
      draco_illixr::ReadPointCloudFromTestFile("float_inf_point_cloud.ply"));
  ASSERT_NE(pc, nullptr);

  {
    draco_illixr::Encoder encoder;
    encoder.SetEncodingMethod(draco_illixr::POINT_CLOUD_SEQUENTIAL_ENCODING);
    encoder.SetAttributeQuantization(draco_illixr::GeometryAttribute::POSITION, 11);
  ...
  {
    draco_illixr::Encoder encoder;
    encoder.SetEncodingMethod(draco_illixr::POINT_CLOUD_KD_TREE_ENCODING);
    encoder.SetAttributeQuantization(draco_illixr::GeometryAttribute::POSITION, 11);
...
TEST_F(EncodeTest, TestQuantizedAndUnquantizedAttributes) {
  ...
  std::unique_ptr<draco_illixr::PointCloud> pc(
      draco_illixr::ReadPointCloudFromTestFile("float_two_att_point_cloud.ply"));
  ASSERT_NE(pc, nullptr);

  draco_illixr::Encoder encoder;
  encoder.SetAttributeQuantization(draco_illixr::GeometryAttribute::POSITION, 11);
  encoder.SetAttributeQuantization(draco_illixr::GeometryAttribute::NORMAL, 0);
  draco_illixr::EncoderBuffer buffer;
  DRACO_ASSERT_OK(encoder.EncodePointCloudToBuffer(*pc, &buffer));
}
...
TEST_F(EncodeTest, TestKdTreeEncoding) {
  ...
  std::unique_ptr<draco_illixr::PointCloud> pc = CreateTestPointCloud();
  ASSERT_NE(pc, nullptr);
...
TEST_F(EncodeTest, TestTrackingOfNumberOfEncodedEntries) {
  TestNumberOfEncodedEntries("deg_faces.obj", draco_illixr::MESH_EDGEBREAKER_ENCODING);
  TestNumberOfEncodedEntries("deg_faces.obj", draco_illixr::MESH_SEQUENTIAL_ENCODING);
  TestNumberOfEncodedEntries("cube_att.obj", draco_illixr::MESH_EDGEBREAKER_ENCODING);
  TestNumberOfEncodedEntries("test_nm.obj", draco_illixr::MESH_EDGEBREAKER_ENCODING);
  TestNumberOfEncodedEntries("test_nm.obj", draco_illixr::MESH_SEQUENTIAL_ENCODING);
  TestNumberOfEncodedEntries("cube_subd.obj",
                             draco_illixr::POINT_CLOUD_KD_TREE_ENCODING);
  TestNumberOfEncodedEntries("cube_subd.obj",
                             draco_illixr::POINT_CLOUD_SEQUENTIAL_ENCODING);
}
...
TEST_F(EncodeTest, TestNoPosQuantizationNormalCoding) {
  ...
  const auto mesh = draco_illixr::ReadMeshFromTestFile("test_nm.obj");
  ASSERT_NE(mesh, nullptr);

  // The mesh should have positions and normals.
  ASSERT_NE(mesh->GetNamedAttribute(draco_illixr::GeometryAttribute::POSITION),
            nullptr);
  ASSERT_NE(mesh->GetNamedAttribute(draco_illixr::GeometryAttribute::NORMAL), nullptr);

  draco_illixr::EncoderBuffer buffer;
  draco_illixr::Encoder encoder;
  // No quantization for positions.
  encoder.SetAttributeQuantization(draco_illixr::GeometryAttribute::NORMAL, 8);
...
TEST_F(EncodeTest, TestDracoCompressionOptions) {
  ...
  // draco_illixr::Mesh's compression options.
  const auto mesh = draco_illixr::ReadMeshFromTestFile("test_nm.obj");
  ASSERT_NE(mesh, nullptr);

  // First set compression level and quantization manually.
  draco_illixr::Encoder encoder_manual;
  draco_illixr::EncoderBuffer buffer_manual;
  encoder_manual.SetAttributeQuantization(draco_illixr::GeometryAttribute::POSITION,
                                          8);
  encoder_manual.SetAttributeQuantization(draco_illixr::GeometryAttribute::NORMAL, 7);
  ...
  draco_illixr::Encoder encoder_auto;
  draco_illixr::EncoderBuffer buffer_auto;
...
TEST_F(EncodeTest, TestDracoCompressionOptionsManualOverride) {
  ...
  // options provided in draco_illixr::Mesh's compression options.
  const auto mesh = draco_illixr::ReadMeshFromTestFile("test_nm.obj");
  ASSERT_NE(mesh, nullptr);
  ...
  draco_illixr::Encoder encoder;
  draco_illixr::EncoderBuffer buffer_no_override;
  ...
  encoder.SetAttributeQuantization(draco_illixr::GeometryAttribute::POSITION, 5);
  draco_illixr::EncoderBuffer buffer_with_override;
...
TEST_F(EncodeTest, TestDracoCompressionOptionsGridQuantization) {
  ...
  // 1x1x1 cube.
  const auto mesh = draco_illixr::ReadMeshFromTestFile("cube_att.obj");
  ASSERT_NE(mesh, nullptr);
  mesh->SetCompressionEnabled(true);
  ...
  const int pos_att_id =
      mesh->GetNamedAttributeId(draco_illixr::GeometryAttribute::POSITION);
  draco_illixr::Vector3f origin;
  encoder.options().GetAttributeVector(pos_att_id, "quantization_origin", 3,
                                       &origin[0]);
  ASSERT_EQ(origin, draco_illixr::Vector3f(0.f, 0.f, 0.f));
...
TEST_F(EncodeTest, TestDracoCompressionOptionsGridQuantizationWithOffset) {
  ...
  auto *pos_att = mesh->attribute(
      mesh->GetNamedAttributeId(draco_illixr::GeometryAttribute::POSITION));
  for (draco_illixr::AttributeValueIndex avi(0); avi < pos_att->size(); ++avi) {
    draco_illixr::Vector3f pos;
    pos_att->GetValue(avi, &pos[0]);
    pos = pos + draco_illixr::Vector3f(-0.55f, 0.65f, 10.75f);
  ...
  ASSERT_EQ(origin, draco_illixr::Vector3f(-0.5625f, 0.625f, 10.75f));
