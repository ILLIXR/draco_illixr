...
TEST_F(DecodeTest, TestSkipAttributeTransform) {
  ...
  ASSERT_TRUE(
      draco_illixr::ReadFileToBuffer(draco_illixr::GetTestFileFullPath(file_name), &data));
  ...
  const draco_illixr::PointAttribute *const pos_att =
      pc->GetNamedAttribute(draco_illixr::GeometryAttribute::POSITION);
  ASSERT_NE(pos_att, nullptr);

  // Ensure the position attribute is of type int32_t and that it has a valid
  // attribute transform.
  ASSERT_EQ(pos_att->data_type(), draco_illixr::DT_INT32);
  ASSERT_NE(pos_att->GetAttributeTransformData(), nullptr);

  // Normal attribute should be left transformed.
  const draco_illixr::PointAttribute *const norm_att =
      pc->GetNamedAttribute(draco_illixr::GeometryAttribute::NORMAL);
  ASSERT_EQ(norm_att->data_type(), draco_illixr::DT_FLOAT32);
...
void TestSkipAttributeTransformOnPointCloudWithColor(const std::string &file) {
  std::vector<char> data;
  ASSERT_TRUE(draco_illixr::ReadFileToBuffer(draco_illixr::GetTestFileFullPath(file), &data));
  ASSERT_FALSE(data.empty());
  ...
  ASSERT_TRUE(pos_att->data_type() == draco_illixr::DT_INT32 ||
              pos_att->data_type() == draco_illixr::DT_UINT32);
  ASSERT_NE(pos_att->GetAttributeTransformData(), nullptr);

  const draco_illixr::PointAttribute *const clr_att =
      pc->GetNamedAttribute(draco_illixr::GeometryAttribute::COLOR);
  ASSERT_EQ(clr_att->data_type(), draco_illixr::DT_UINT8);
  ...
  draco_illixr::DecoderBuffer buffer_2;
  buffer_2.Init(data.data(), data.size());

  draco_illixr::Decoder decoder_2;

  // Decode the input data into a geometry.
  std::unique_ptr<draco_illixr::PointCloud> pc_2 =
      decoder_2.DecodePointCloudFromBuffer(&buffer_2).value();
  ASSERT_NE(pc_2, nullptr);

  const draco_illixr::PointAttribute *const clr_att_2 =
      pc_2->GetNamedAttribute(draco_illixr::GeometryAttribute::COLOR);
  ASSERT_NE(clr_att_2, nullptr);
  for (draco_illixr::PointIndex pi(0); pi < pc_2->num_points(); ++pi) {
...
TEST_F(DecodeTest, TestSkipAttributeTransformWithNoQuantization) {
  ...
  // not quantized.
  ASSERT_EQ(pos_att->data_type(), draco_illixr::DT_FLOAT32);
...
TEST_F(DecodeTest, TestSkipAttributeTransformUniqueId) {
  ...
  auto src_mesh = draco_illixr::ReadMeshFromTestFile(file_name);
  ...
  src_mesh
      ->attribute(
          src_mesh->GetNamedAttributeId(draco_illixr::GeometryAttribute::POSITION))
      ->set_unique_id(kPosUniqueId);
  src_mesh
      ->attribute(
          src_mesh->GetNamedAttributeId(draco_illixr::GeometryAttribute::NORMAL))
      ->set_unique_id(kNormUniqueId);

  draco_illixr::EncoderBuffer encoder_buffer;
  draco_illixr::Encoder encoder;
  encoder.SetAttributeQuantization(draco_illixr::GeometryAttribute::POSITION, 10);
  encoder.SetAttributeQuantization(draco_illixr::GeometryAttribute::NORMAL, 11);
  ...
  draco_illixr::Decoder decoder_no_skip;
  std::unique_ptr<draco_illixr::Mesh> mesh_no_skip =
  ...
  draco_illixr::Decoder decoder_skip;
  // Make sure we skip dequantization for the position and normal attribute.
  decoder_skip.SetSkipAttributeTransform(draco_illixr::GeometryAttribute::POSITION);
  decoder_skip.SetSkipAttributeTransform(draco_illixr::GeometryAttribute::NORMAL);

  // Decode the input data into a geometry.
  buffer.Init(encoder_buffer.data(), encoder_buffer.size());
  std::unique_ptr<draco_illixr::Mesh> mesh_skip =
      decoder_skip.DecodeMeshFromBuffer(&buffer).value();
  ASSERT_NE(mesh_skip, nullptr);

  // Compare the unique ids.
  const draco_illixr::PointAttribute *const pos_att_no_skip =
      mesh_no_skip->GetNamedAttribute(draco_illixr::GeometryAttribute::POSITION);
  ASSERT_NE(pos_att_no_skip, nullptr);
  ASSERT_EQ(pos_att_no_skip->data_type(), draco_illixr::DataType::DT_FLOAT32);

  const draco_illixr::PointAttribute *const pos_att_skip =
      mesh_skip->GetNamedAttribute(draco_illixr::GeometryAttribute::POSITION);
  ASSERT_NE(pos_att_skip, nullptr);
  ASSERT_EQ(pos_att_skip->data_type(), draco_illixr::DataType::DT_INT32);

  const draco_illixr::PointAttribute *const norm_att_no_skip =
      mesh_no_skip->GetNamedAttribute(draco_illixr::GeometryAttribute::NORMAL);
  ASSERT_NE(norm_att_no_skip, nullptr);
  ASSERT_EQ(norm_att_no_skip->data_type(), draco_illixr::DataType::DT_FLOAT32);

  const draco_illixr::PointAttribute *const norm_att_skip =
      mesh_skip->GetNamedAttribute(draco_illixr::GeometryAttribute::NORMAL);
  ASSERT_NE(norm_att_skip, nullptr);
  ASSERT_EQ(norm_att_skip->data_type(), draco_illixr::DataType::DT_INT32);
