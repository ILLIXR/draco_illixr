...
class AttributesEncoder {
 public:
  ...
  virtual bool EncodeAttributes(EncoderBuffer *out_buffer) {
    //printf("reached encodeAttributes\n");
    if (!TransformAttributesToPortableFormat()) {
      return false;
    }
    //printf("finished transform attributes to portable format\n");
    if (!EncodePortableAttributes(out_buffer)) {
      return false;
    }
    //printf("finished encode portable attributes\n");
    ...
    //printf("finished encode data needed by portable transforms\n");
    return true;
  }
