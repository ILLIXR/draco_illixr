...
bool PointCloudEncoder::EncodePointAttributes() {
  if (!GenerateAttributesEncoders()) {
     printf("failed to generate attributes encoder\n");
    return false;
  }

  // Encode the number of attribute encoders.
  //printf("reached buffer encode, attribute encoder size %zu\n", attributes_encoders_.size());
  buffer_->Encode(static_cast<uint8_t>(attributes_encoders_.size()));
  //printf("finish buffer encode\n");
  ...
  for (auto &att_enc : attributes_encoders_) {
    if (!att_enc->Init(this, point_cloud_)) {
      printf("initialize encoder failure\n");
      return false;
    }
  }
  //printf("finish initialize all encoders\n");
  // Rearrange attributes to respect dependencies between individual attributes.
  if (!RearrangeAttributesEncoders()) {
    printf("rearrange encoder failure\n");
    return false;
  }
  //printf("finish rearrange attributes\n");
  ...
  for (int att_encoder_id : attributes_encoder_ids_order_) {
      //printf("att encoder id %d\n", att_encoder_id);
    if (!EncodeAttributesEncoderIdentifier(att_encoder_id)) {
      printf("faile to encode attributes encoder identifier\n");
      return false;
    }
  }
  //printf("finish encode any data\n");
  ...
  for (int att_encoder_id : attributes_encoder_ids_order_) {
    if (!attributes_encoders_[att_encoder_id]->EncodeAttributesEncoderData(
            buffer_)) {
        printf("failed to  encode attributesEncoderData\n");
        return false;
    }
  }
  //printf("finish encode attribute data\n");

  // Lastly encode all the attributes using the provided attribute encoders.
  if (!EncodeAllAttributes()) {
    printf("failed to encode all attributes\n");
    return false;
  }
  //printf("reached the end of encodepointattributes\n");
  return true;
}
...
bool PointCloudEncoder::EncodeAllAttributes() {
  //printf("reached encodeallattributes\n");
  for (int att_encoder_id : attributes_encoder_ids_order_) {
    //printf("encode all attribute id %d\n", att_encoder_id);
    if (!attributes_encoders_[att_encoder_id]->EncodeAttributes(buffer_)) {
      printf("failed in encode all attributes\n");
      return false;
    }
  }
  //printf("finished encode all attributes\n");
  return true;
}
